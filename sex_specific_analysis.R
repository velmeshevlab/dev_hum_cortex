...
get_sex_enriched_genes(exneu_sex, factor = 1.2, window_ratio = 0.01, I = 0.1)
...
Interneurons
#######################################
IN = readRDS(file = "IN.RDS")
#isolate male cells and compress lineages
cells = rownames(colData(IN)[colData(IN)$sex == "Male",])
cds.male = IN[,cells]
cds.male = get_lineage_object(cds.male, start = 203)
cds.male <- compress_lineages_v2(cds.male, start = 203, cores = 24)
#female
cells = rownames(colData(IN)[colData(IN)$sex == "Female",])
cds.female = IN[,cells]
cds.female = get_lineage_object(cds.female, start = 203)
cds.female <- compress_lineages_v2(cds.female, start = 203, cores = 24)
...
options(mc.cores = 1)
print(lineage)
cds.sub = get_lineage_object(cds.male, lineage, 203, N = N)
data = counts(cds.sub)
rows=rownames(data)[rowSums(data> 0) > factor*ncol(data)]
cds.sel = cds.sub[rows]
print(paste0("Testing ", ncol(cds.sel)," cells and ", nrow(cds.sel), " genes"))
...
factor = 0.05
N = 10000
lineages = names(cds.female@lineages)
for(lineage in lineages){
options(mc.cores = 1)
print(lineage)
cds.sub = get_lineage_object(cds.female, lineage, 203, N = N)
data = counts(cds.sub)
rows=rownames(data)[rowSums(data> 0) > factor*ncol(data)]
cds.sel = cds.sub[rows]
print(paste0("Testing ", ncol(cds.sel)," cells and ", nrow(cds.sel), " genes"))
...
in_sex = combine_objects(cds.male, cds.female, "_Male", "_Female")
get_sex_enriched_genes(in_sex, factor = 1.2, window_ratio = 0.01, I = 0.1)
